...
const Login = () => {
  ...
  const setCurrentUser = useUserStore((state) => state.setCurrentUser); // optional for user switching

  // Sync Supabase session on refresh or app load
  useEffect(() => {
    ...
    // Listen to session changes (Google login / logout)
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_IN" && session) {
        ...
        useUserStore.getState().logout?.(); // optional if you have logout in store
  ...
  // Maps Supabase user to your custom user object
  const mapUser = (user) => ({
    id: user.id,
    email: user.email,
    username:
      user.user_metadata.full_name?.toLowerCase().replace(/\s+/g, "") || user.email.split("@")[0],
